$( document ).ready(function() {
    var table = $('#hellpass-table').DataTable({
        ...
        [
            ...
            },
            {
                ...
                render: function ( value, type, row ) {
                    return `
                        <input class="form-control" type="text" value="${value}" name="name-${row.pk}"readonly> 
                        `
                },
            },
            {
                ...
                render: function ( value, type, row ) {
                    return `
                        <input class="form-control" type="text" value="${value}" name="url-${row.pk}" readonly> 
                        `
                },
            },
            { 
                ...
                render: function ( value, type, row ) {
                    return `
                        <input class="form-control" type="text" value="${value}" name="username-${row.pk}" readonly> 
                        `
                },
            },
            { 
                ...
                render: function ( value, type, row ) {
                    return `
                        <input class="form-control" type="password" value="${value}" name="password-${row.pk}" id="password-${row.pk}" readonly>  
                        `
                },
            },
            { 
        ...
        drawCallback: function () {
            ...
            $('#hellpass-new').unbind().click(function(pk) {
                console.log('New')
                pk = modalParser(+$('tr:last td:first input').val() + 1);
                newPassword();
                showSettings(pk);                
            });
    ...
    var modalParser = function (password_pk) {
        ...
        username = $(`input[name=username-${password_pk}]`).val()
        password = $(`input[name=password-${password_pk}]`).val()
        ...
        modal_html = `
            <div class="modal-header">
                ...
                <h4 class="modal-title"> ${password_pk} - ${name} - ${url}</h4>
            </div>
            <div class="modal-body" id="modal-body">
                Name:
                <input name="name" class="form-control" value="${name}">
                URL:
                <input name="url" class="form-control" value="${url}">
                Username: 
                <input name="username" class="form-control" value="${username}">
                Password:
                <input name="password" class="form-control" value="${password}">
    ...
    var updateExpires = function (password_pk, expires, name, url, username, password) {
        if (expires == 0) {
        ...
        $(`input[name=name-${password_pk}]`).val(name)
        $(`input[name=url-${password_pk}]`).val(url)
        $(`input[name=username-${password_pk}]`).val(username)
        $(`input[name=password-${password_pk}]`).val(password)
    ...
    var showSettings = function (modal_html) {
        ...
        $('#modal-save').on('click', function(){
            ...
            updateExpires(password_pk, expires, name, url, username, password)
        });
    };
    ...
    $("div.toolbar").html(`<div style="float:left; margin-left: 15px; margin-bottom: 15px;">
        <button class="btn btn-primary " type="button" id="hellpass-new">+ New</button>
